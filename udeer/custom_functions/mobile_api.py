...
def activity(customer):
  customer = json.loads(customer)
  leases = frappe.get_list('lease',fields=["*"], filters = [['renter','=',customer]] )
  lease_instalments = frappe.get_list('lease_instalment',fields=["*"], filters = [['lease','in',dic_to_array(leases)],['status','=','paid']])
  maintenance_order = frappe.get_list('Issue',fields=["*"], filters = [['Customer','=', customer ],['type','=', 'maintenance']])
...
def my_leases(customer):
  customer = json.loads(customer)
  expired_leases = frappe.get_list('lease',fields=["*"], filters = [['renter','=',customer],['active','=',0]] )
  active_leases = frappe.get_list('lease',fields=["*"], filters = [['renter','=',customer],['active','=',1]] )
...
def leases_report(owner):
  owner = json.loads(owner)
  properties = frappe.get_list('property',filters = [['property_owner','=',owner]])
...
def property_data(owner):
  ...
  units = frappe.get_list('property unit',filters = [['property_owner','=',owner]])
...
def open_maintenance_issues(owner):
  ...
  maintenance_list = frappe.get_list('Issue',fields=["*"], filters = [['property_unit','in', property_units ],['type','=', 'maintenance'],['status','=', 'Open']])
